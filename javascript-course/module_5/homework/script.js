...
const getAllUniqueSkills = (arr) => 
  arr.reduce((acc, item) => item.skills ? acc.concat(item.skills) : acc , [])
  .reduce((acc, item) => !acc.includes(item) ? acc.concat(item) : acc , [])
  .sort();
...
const getUserNamesSortedByFriendsCount = arr => 
  arr.sort((first, second) => first.friends.length - second.friends.length)
  .map(user => user.name);
