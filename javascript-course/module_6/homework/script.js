...
class Hamburger {
   ...
   */
  addTopping(topping) {            
    if ( !this.toppings.includes(topping) ) this._toppings.push(topping);
};
  ...
  removeTopping(topping) {
    if ( this.toppings.includes(topping) ) this.toppings.splice( this.toppings.indexOf(topping), 1); 
  }
   ...
   */
  get price () { return this.calculatePrice() }
  calculatePrice () {
    const sizePrice = Hamburger.SIZES[this.size].price;
    const stuffingPrice = Hamburger.STUFFINGS[this.stuffing].price;
    const toppingPrice = this.toppings.reduce((acc, topping) => acc + Hamburger.TOPPINGS[topping].price, 0);            
  ...
  calculateCalories () {
    ...
    const toppingCalories = this.toppings.reduce((acc, el) => acc + Hamburger.TOPPINGS[el].calories, 0);
